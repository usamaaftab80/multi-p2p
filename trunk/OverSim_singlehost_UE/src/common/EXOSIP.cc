					...
					// fetch MESSAGE body, get nodeID to handle
					 ...
					 osip_body_t * oldbody;
					   while (!osip_list_eol (&event->request->bodies, pos))
					   {
					       ...
					       sscanf(oldbody->body,"%s",type,str);
					       if( string(type) == "HOANG_SERVER_BEGIN" ){
							   cout << localSocket << " get a HOANG_SERVER_BEGIN at " << simTime() << endl;
							   globalObject->setServerBeginTime(simTime().dbl());
						   }
