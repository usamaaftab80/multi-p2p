...
public class ConferenceServlet extends SipServlet {
    ...
    public void init(ServletConfig servletConfig) throws ServletException {
		...
		en[0].seteNodeB(1,"sip:enodeB1@open-ims.test","157.159.16.91","6080");
	...
	protected void doMessage(SipServletRequest req) throws ServletException,
		...
		//SipServletRequest req; //Request for Overlay network 
		String toAddress= "157.159.16.122"; 
		String toPort ="5080"; 
		...
		SipURI requestUri = (SipURI) _sipFactory.createURI("sip:singlehost@157.159.16.122:5080");
		...
		if (temp1[0].contains("HEARTBIT")){
				logger.info("Receive message : HEART BIT"); 
				logger.info("Send Overlay message: HEART BIT"); 
				conMessage = "HEART BIT\n";
				...
				for (int i=0; i<en[1].nbrParticipant;i++){
					conMessage = conMessage+"ideNodeB="+en[1].ID+ 
					             "\nid="+i+1+
					             "\nuri="+en[1].user[i+1].userURI+
					             "\nrole="+en[1].user[i+1].role+
					             "\nstatus="+en[1].user[i+1].status;  
				}
		...
		if (temp1[0].contains("HANDOVER")){
			...
			for (int i=1;i<conference.nbrParticipant+1; i++){
				if (fromUser.contains(conference.userURI[i])){
					logger.info("============== Tim duoc User:"+fromUser + "== " + conference.userURI[i]);
					logger.info("============== Chuyen User tu "+en[conference.idEnodeB[i]].eNodeBURI +"sang new enodeB ");
					...
					logger.info("============== User thuoc enodeB moi : "+en[conference.idEnodeB[i]].eNodeBURI);
					...
					conMessage = "HEART BIT\n";
					for (int k=1; i<en[0].nbrParticipant+1;k++){
						conMessage = conMessage+"ideNodeB="+en[0].ID+ 
						             "\nid="+k+1+
						             "\nuri="+en[0].user[k].userURI+
						             "\nrole="+en[0].user[k].role+
						             "\nstatus="+en[0].user[k].status;  
					}
					logger.info("============== Test2 ");
					for (int k=1; k<en[1].nbrParticipant+1;k++){
						conMessage = conMessage+"ideNodeB="+en[1].ID+ 
						             "\nid="+k+1+
						             "\nuri="+en[1].user[k].userURI+
						             "\nrole="+en[1].user[k].role+
						             "\nstatus="+en[1].user[k].status;
	...
	IOException {
		...
		//15072010
		// Fix enode B
		//eNodeB en1 = new eNodeB();
		//en1.seteNodeB(1, "sip:enodeB1@open-ims.test");
...
/*			
			...
			for (int j = 0; j<2; j++){
				for (int i = 0; i < en[j].nbrParticipant+1; i++) {
					if (en[j].user[i+1].userURI.equals(from.toString())) {
						...
						logger.info("Test Bye 1 - enodeB has: "+ en[j].nbrParticipant+" participants");
					}			
				}
			...
			for (int i = 1; i < nbrUser+1; i++){
				...
				logger.info("User " + conference.userURI[i]+" in enode B"+ conference.idEnodeB[i]);
			}
			
			for (int i = 1; i < nbrUser+1; i++) {			
				if (conference.userURI[i].equals(from.toString())){
					logger.info("Test Bye 2 - Remove User " + conference.userURI[i]+ " in enodeB "+ conference.idEnodeB[i]);				
...
*/			
	...
	protected void doInvite(SipServletRequest request) throws ServletException,
		...
		//SipServletRequest req; //Request for Overlay network 
		String toAddress= "157.159.16.91"; 
		String toPort ="8000"; 
		//String fromAddress= "157.159.16.91";
		//String fromPort ="5080";
		//String fromAddress = request.getAddressHeader("From").toString();
		...
		for (int i = 0; i< temp1.length; i++){
			...
			if (temp1[i].contains("m=audio")){
				String [] temp2 = null;
				temp2 = temp1[i].split(" ");
				logger.info("Port Media:"+temp2[1]);
				portMedia = temp2[1];
			}
		}
	...
	protected void doBye(SipServletRequest request) throws ServletException,
		...
		SipURI toOverlay = _sipFactory.createSipURI("singlehost", toAddress); 
		SipURI fromOverlay = _sipFactory.createSipURI("enodeB1", fromAddress); 
