...
int sed_pels, enhancement_sed_pels, base_sed_pels;
int csed_pels;
int sed_lines, enhancement_sed_lines, base_sed_lines;
int trace;
int adv_pred;
int use_4mv;
int overlapping_MC;
...
void MakeEdgeImage(unsigned char *src, unsigned char *dst, int width,
           int height, int edge);
void sed_UpsampleComponent (unsigned char *base, unsigned char *enhanced,
                        int horiz, int vert);
PictImage *sed_UpsampleReferenceLayerPicture(PictImage *base_image);
...
#ifndef FASTIDCT
/* global declarations for sed_idctref */
void sed_init_sed_idctref (void);
void sed_idctref (int *coeff, int *block);
#endif
...
void sed_fill_A(int pred[][6],int *store_qcoeff, int xpos,int ypos, int block, int i);
void sed_fill_B(int  pred[][6],int *store_qcoeff, int xpos,int ypos, int block, int i);
void Intra_AC_DC_Encode(int *pcoeff, int *store_rcoeff, int INTRA_AC_DC, int xpos, int ypos, int newgob, int i);
void sed_Intra_AC_DC_Decode(int *rcoeff, int *store_qcoeff, int INTRA_AC_DC, int xpos, int ypos, int newgob, int i);
