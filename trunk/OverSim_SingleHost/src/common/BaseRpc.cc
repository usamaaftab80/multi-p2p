...
void BaseRpc::internalHandleRpcMessage(BaseRpcMessage* msg)
{
    ...
    if (rpCall != NULL) {
        ...
        if (overlayCtrlInfo->getSrcRoute().isUnspecified() &&
    ...
    if (msg->isSelfMessage() &&
        ...
        if (state.transportType == UDP_TRANSPORT ||
            ...
            if (neighborCache->vivaldiEnabled()) {
                // vivaldi: get coordinates/error estimation from message
                std::vector<double> nodeCoords;
                nodeCoords.resize(response->getCoordsArraySize());
                for (uint32_t i = 0; i < response->getCoordsArraySize(); i++)
                    nodeCoords[i] = response->getCoords(i);
                              ...
                              BaseResponseMessage* response)
{
    ...
    if (neighborCache->vivaldiEnabled()) { //TODO only for directly sent msgs
        const std::vector<double>& nodeCoord =
            neighborCache->getVivaldiAccess().getOwnCoordinates();
        response->setError(neighborCache->getVivaldiAccess().getOwnError());
        response->setCoordsArraySize(nodeCoord.size());
        for (uint32_t i = 0; i < nodeCoord.size(); i++) {
            response->setCoords(i, nodeCoord[i]);
        }
    }
                       ...
                       int rpcId, TransportType transportType)
{
    ...
    if (transportType == UDP_TRANSPORT ||
        ...
        sendUdpRpcCall(dest, call, context, timeout, retries, rpcId,
                       rpcListener);
    } else {
        sendRouteRpcCall(getThisCompType(), dest, call, context,
...
//Prox BaseRpc::getProx(const TransportAddress &node,
//                      int type,
//                      NeighborCacheQueryType /*int*/ rpcId,
//                      ProxListener *listener,
//                      cPolymorphic *contextPointer)
//{
//    if (neighborCache) {
//        return neighborCache->getProx(node, (NeighborCacheQueryType)type,
//                                      rpcId, listener, contextPointer);
//    } else {
//        //TODO
//        pingNode(node, -1, 0, contextPointer, NULL, NULL,
//                 rpcId, UDP_TRANSPORT, true);
