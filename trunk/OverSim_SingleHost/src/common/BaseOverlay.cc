...
void BaseOverlay::initialize(int stage)
{
    ...
    if (stage == MIN_STAGE_OVERLAY) {
        ...
        iterativeLookupConfig.secure = par("lookupSecure");
        ...
        */
        ...
        thisNode.setAddress(IPAddressResolver().
                      addressOf(getParentModule()->getParentModule()).get4());
        ...
        // statistics
        globalStatistics->nodesInitialized++;
    ...
    // join after all components are registered
    if (stage == MAX_STAGE_TIER_3) {
...
void BaseOverlay::finish()
{
    ...
    if (time >= GlobalStatistics::MIN_MEASURED) {
        ...
        globalStatistics->addStdDev("BaseRpc: Sent Ping Messages/s",
                                    numPingSent / time);
        globalStatistics->addStdDev("BaseRpc: Sent Ping Bytes/s",
                                    bytesPingSent / time);
                              ...
                              const NodeHandle& nextHopNode)
{
    ...
    forwardMsg->encapsulate(msg->getEncapsulatedMsg()->decapsulate());
    ...
    overlayCtrlInfo->setSrcComp(check_and_cast<BaseAppDataMessage*>
        (msg->getEncapsulatedMsg())->getSrcComp());
    overlayCtrlInfo->setDestComp(check_and_cast<BaseAppDataMessage*>
        (msg->getEncapsulatedMsg())->getDestComp());
...
void BaseOverlay::join(const OverlayKey& nodeID)
{
    ...
    if (state != READY) {
        // set nodeID and IP
        thisNode.setAddress(
            IPAddressResolver().addressOf(getParentModule()->getParentModule()).get4());
...
//        kw = udpControlInfo->getMinBW();
    ...
    // process CommonAPIMessages from App
        ...
        if (dynamic_cast<KBRroute*>(msg) != NULL) {
            ...
            dataMsg->setName(apiMsg->getEncapsulatedMsg()->getName());
    ...
    else if (msg->getArrivalGate() == appGate/*msg->arrivedOn("appIn")*/) {
...
void BaseOverlay::handleTransportAddressChangedNotification()
{
    ...
    thisNode.setAddress(IPAddressResolver().addressOf(
                      getParentModule()->getParentModule()).get4());
                                   ...
                                   bool ack)
{
    ...
    if (!ack)
        ...
        sendUdpRpcCall(dest, nextHopCall);
    }
}
                        ...
                        RoutingType routingType)
{
    ...
    // debug output
    if (debugOutput) {
        EV << "[BaseOverlay::getRoute() @ " << thisNode.getAddress()
        << " (" << thisNode.getKey().toString(16) << ")]\n"
...
{
    if (dynamic_cast<FindNodeCall*>(routeMsg->getEncapsulatedMsg())) {
