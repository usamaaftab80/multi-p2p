...
#include <GlobalStatistics.h>//hoang
...
TransportAddress* InetUnderlayConfigurator::createNode(NodeType type, bool initialize)
{
    ...
    InetInfo* info = new InetInfo(type.typeID, node->getId());
...
void InetUnderlayConfigurator::migrateNode(NodeType type, TransportAddress* addr)
{
    ...
    // create meta information
    InetInfo* newinfo = new InetInfo(type.typeID, node->getId());
    // add node to a randomly chosen access net
...
{
    //globalStatistics->recordOutVector("HOANG so lan goi finish underlay",1);
    //std::cout << "finish underlayyyyyyyyyy" << endl;
