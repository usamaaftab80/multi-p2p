...
void Vast::changeState(int state)
{
    ...
    // debug output
    if(debugOutput) {
        EV << "[Vast::changeState() @ " << thisSite.addr.getAddress()
           << " (" << thisSite.addr.getKey().toString(16) << ")]\n"
           << "VAST: Node " << thisSite.addr.getAddress() << " entered ";
        switch(state) {
...
void Vast::handleAppMessage(cMessage* msg)
{
    if(dynamic_cast<GameAPIMessage*>(msg)) {
        ...
        // debug output
        if(debugOutput) EV << "[Vast::handleAppMessage() @ " << thisSite.addr.getAddress()
                           << " (" << thisSite.addr.getKey().toString(16) << ")]\n"
                           << "    Node " << thisSite.addr.getAddress() << " received " << gameAPIMsg->getName() << " from application."
                           << endl;
    ...
    if(dynamic_cast<VastMessage*>(msg)) {
        ...
        if(vastMsg->getDestKey().isUnspecified() || 
           ...
           vastMsg->getDestKey() == thisSite.addr.getKey()) {
            // debug output
            if(debugOutput) EV << "[Vast::handleUDPMessage() @ " << thisSite.addr.getAddress()
                               << " (" << thisSite.addr.getKey().toString(16) << ")]\n"
                               << "    Node " << thisSite.addr.getAddress() << " received " << vastMsg->getName() << " from " << vastMsg->getSourceNode().getAddress()
                               << endl;
            bool doUpdate = true;
            if(state == READY) {
...
void Vast::buildVoronoi(Vector2D old_pos, Vector2D new_pos)
{
    int sqrt_nsites = 1;
...
void Vast::handleNodeLeaveNotification()
{
    if(state == READY) {
        // debug output
        if(debugOutput) {
            EV << "[Vast::receiveChangeNotification() @ " << thisSite.addr.getAddress()
               << " (" << thisSite.addr.getKey().toString(16) << ")]\n"
               << "    Node " << thisSite.addr.getAddress() << " is leaving the overlay."
               << endl;
        }
...
void Vast::handleNodeMove(VastMoveMessage *vastMoveMsg)
{
    ...
    if(vastMoveMsg->getIs_boundary() || vastMoveMsg->getRequest_list()) {
        if(!vastMoveMsg->getRequest_list()) {
            buildVoronoi(old_p, new_p);
...
void Vast::sendDiscardNode(VastMessage *vastMsg)
{
    ...
    discardNode.setAddress(thisSite.addr.getAddress());
    ...
    // debug output
    if(debugOutput) EV << "[Vast::sendDiscardNode() @ " << thisSite.addr.getAddress()
                       << " (" << thisSite.addr.getKey().toString(16) << ")]\n"
                       << "    Node " << thisSite.addr.getAddress() << " is leaving the overlay."
                       << endl;
...
{
    // debug output
    if(debugOutput) EV << "[Vast::sendToApp() @ " << thisSite.addr.getAddress()
                       << " (" << thisSite.addr.getKey().toString(16) << ")]\n"
                       << "    Node " << thisSite.addr.getAddress() << " sending " << msg->getName() << " to application."
                       << endl;
    send(msg, "appOut");
}
...
void Vast::sendMessage(VastMessage *vastMsg, NodeHandle destAddr)
{
    ...
    // debug output
    if(debugOutput) EV << "[Vast::sendMessage() @ " << thisSite.addr.getAddress()
                       << " (" << thisSite.addr.getKey().toString(16) << ")]\n"
                       << "    Node " << thisSite.addr.getAddress() << " sending " << vastMsg->getName() << " to " << destAddr.getAddress() << "."
                       << endl;
