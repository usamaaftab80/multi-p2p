...
bool BrooseBucket::add(const NodeHandle& node, bool isAlive, simtime_t rtt)
{
    ...
    if (bucketIter == bucket.end()) {
        ...
        if (isBBucket && (getPos(node) < (maxSize/7))) {
            // node is a new sibling
            if (bucket.size() > (maxSize/7)) {
                // call update() for removed sibling
                overlay->callUpdate(get(maxSize/7), false);
            }
...
void BrooseBucket::remove(const NodeHandle& node)
{
    ...
    for (bucketIter = bucket.begin(); bucketIter != bucket.end(); i++) {
        if (bucketIter->second.getAddress() == node.getAddress()) {
...
void BrooseBucket::output(int maxEntries)
{
    ...
    EV << "[BrooseBucket::output() @ " << overlay->getThisNode().getAddress()
    ...
    for (bucketIter = bucket.begin(), i = 0; i < max; bucketIter++, i++) {
        ...
        EV << "    " << dist << " " << node.getKey() << " " << node.getAddress() << " RTT: "
...
{
    int i = -1;
    for (bucketIter = bucket.begin(); bucketIter!=bucket.end(); bucketIter++, i++) {
        if (node.getAddress() == bucketIter->second.getAddress())
            return i;
    }
    return i;
}
