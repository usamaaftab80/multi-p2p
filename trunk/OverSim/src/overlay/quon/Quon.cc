...
void Quon::initializeOverlay(int stage)
{
    ...
    // fetch parameters
    minAOI = (double)par("minAOIWidth") * (double)par("AOIBufferFactor"); // FIXME: use buffer only where required
    maxAOI = (double)par("AOIWidth") * (double)par("AOIBufferFactor"); // FIXME: use buffer only where required
    AOIWidth = maxAOI;
    ...
    AOIAdaptionIntervall = par("AOIAdaptionIntervall");
    ...
    useDynamicAOI = (connectionLimit > 0 && minAOI < maxAOI && AOIAdaptionIntervall > 0.0) ? true : false;
    ...
    adaption_timer = new cMessage("adaption_timer");
...
void Quon::changeState(QState qstate)
{
    ...
    switch(qstate) {
        ...
        case QREADY:
            ...
            if(useDynamicAOI) {
                scheduleAt(simTime() + AOIAdaptionIntervall, adaption_timer);
            }
            break;
    }
    setBootstrapedIcon();
    // debug output
    if(debugOutput) {
        EV << "[Quon::changeState() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Node " << thisSite->address.getAddress() << " entered ";
        switch(qstate) {
            case QUNINITIALIZED:
                EV << "UNINITIALIZED";
                break;
...
void Quon::handleTimerEvent(cMessage* msg)
{
    ...
    else if(msg->isName("adaption_timer")) {
        //reset timer
        cancelEvent(adaption_timer);
        scheduleAt(simTime() + AOIAdaptionIntervall, msg);
        // adjust AOIWidth
#if 0
        // Old simple adaption
        if((Sites.size() > connectionLimit && AOIWidth > minAOI) || (Sites.size() < connectionLimit && AOIWidth < maxAOI)) {
            AOIWidth -= (maxAOI - minAOI) * (Sites.size() - connectionLimit) / 200.0;
            if(AOIWidth > maxAOI) {
                AOIWidth = maxAOI;
            }
...
void Quon::handleAppMessage(cMessage* msg)
{
    ...
    if(gameAPIMsg != NULL) {
        // debug output
        if(debugOutput) {
            EV << "[Quon::handleAppMessage() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    Node " << thisSite->address.getAddress() << " received " << gameAPIMsg->getName() << " from application."
               << endl;
        }
...
void Quon::handleUDPMessage(BaseOverlayMessage* msg)
{
    ...
    if(quonMsg != NULL) {
        // debug output
        if(debugOutput) {
            EV << "[Quon::handleUDPMessage() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    Node " << thisSite->address.getAddress() << " received " << quonMsg->getName() << " from " << quonMsg->getSender().getAddress() << "."
               << endl;
        }
        if(qstate == QREADY) {
...
bool Quon::addSite(Vector2D p, NodeHandle node, double AOI, bool isSoft, QUpdateType update)
{
    ...
    if(node.getKey() != thisSite->address.getKey() && deletedSites.find(node.getKey()) == deletedSites.end()) {
        if(itSites == Sites.end()) {
            if(debugOutput) {
                EV << "[Quon::addSite() @ " << thisNode.getAddress()
                   << " (" << thisNode.getKey().toString(16) << ")]\n"
                   << "    Site " << node.getAddress() << " at " << p << " has been added to the list."
                   << endl;
            }
            ...
            if(debugOutput) {
                EV << "[Quon::addSite() @ " << thisNode.getAddress()
                   << " (" << thisNode.getKey().toString(16) << ")]\n"
                   << "    Site " << node.getAddress() << " at " << p << " has been updated in the list."
                   << endl;
            }
...
{
    if(debugOutput) {
        EV << "[Quon::updateThisSite() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
...
void Quon::classifySites()
{
    if(Sites.size() > 0) {
        ...
        QuonSite* bindings[4] = {0, 0, 0, 0};
        QuonSite* backupCandidates[4] = {0, 0, 0, 0};
        double bindingDistance[4] = {std::numeric_limits<double>::infinity(),
        ...
        for(QuonSiteMap::iterator itSites = Sites.begin(); itSites != Sites.end(); ++itSites) {
            ...
            if( dist < bindingDistance[quad] ){
               backupCandidates[quad] = bindings[quad];
               bindings[quad] = itSites->second;
               bindingDistance[quad] = dist;
            }
        }
        for( int i = 0; i < 4; ++i ){
            if( bindings[i] ){
                bindings[i]->type = QBINDING;
                bindings[i]->dirty = true;
//                if(!AOI.collide(bindings[i]->position)) {
//                    bindingBackup[i] = bindings[i]->address;
                if( backupCandidates[i] ){
                    bindingBackup[i] = backupCandidates[i]->address;
                }
            }
        }
    }
...
bool Quon::deleteSite(NodeHandle node)
{
    ...
    if(itSites != Sites.end()) {
        if(debugOutput) {
            EV << "[Quon::deleteSite() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    Site " << node.getAddress() << " at " << itSites->second->position << " has been removed from the list."
               << endl;
        }
...
int Quon::purgeSites(QPurgeType purgeSoftSites)
{
    ...
    while(itSites != Sites.end()) {
        ...
        if(itSites->second->type == QUNDEFINED && ( purgeSoftSites == QPURGESOFT || !itSites->second->softNeighbor) ) {
            if(debugOutput) {
                EV << "[Quon::purgeSites() @ " << thisNode.getAddress()
                   << " (" << thisNode.getKey().toString(16) << ")]\n"
                   << "    Site " << itSites->second->address.getAddress() << " at " << itSites->second->position << " has been removed from the list.\n"
...
void Quon::handleNodeLeave(QuonListMessage* quonListMsg)
{
    ...
    deletedSites.insert(std::make_pair(quonListMsg->getSender().getKey(), msg));
...
void Quon::sendToApp(cMessage* msg)
{
    // debug output
    if(debugOutput) {
        EV << "[Quon::sendToApp() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Node " << thisSite->address.getAddress() << " sending " << msg->getName() << " to application."
           << endl;
    }
    send(msg, "appOut");
}
...
void Quon::sendMessage(QuonMessage* quonMsg, NodeHandle destination)
{
    ...
    // debug output
    if(debugOutput) {
        EV << "[Quon::sendMessage() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Node " << thisSite->address.getAddress() << " sending " << quonMsg->getName() << " to " << destination.getAddress() << "."
           << endl;
    }
...
{
    Enter_Method_Silent();
    return AOIWidth / (double)par("AOIBufferFactor");
}
...
Quon::~Quon()
{
    ...
    cancelAndDelete(adaption_timer);
    delete thisSite;
    QuonSiteMap::iterator itSites = Sites.begin();
    while(itSites != Sites.end()) {
        delete itSites->second;
