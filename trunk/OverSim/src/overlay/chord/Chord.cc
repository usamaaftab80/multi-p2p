...
void Chord::changeState(int toState)
{
    ...
    case INIT:
        ...
        // debug message
        if (debugOutput) {
            EV << "[Chord::changeState() @ " << thisNode.getAddress()
...
{
    if (state != READY) {
        EV << "[Chord::handleRpcCall() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
                              ...
                              simtime_t rtt)
{
    ...
    RPC_ON_RESPONSE( Join ) {
        ...
        EV << "[Chord::handleRpcResponse() @ " << thisNode.getAddress()
        << " (" << thisNode.getKey().toString(16) << ")]\n"
                             ...
                             const OverlayKey&)
{
    ...
    RPC_ON_CALL( FindNode ) {
        EV << "[Chord::handleRpcTimeout() @ " << thisNode.getAddress()
        << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    for (int i = fingerTable->getSize() - 1; i >= 0; i--) {
        ...
        if (fingerTable->getFinger(i).getKey().isBetweenLR(tempHandle.getKey(), key)) {
            if(!extendedFingerTable) {
                ...
                EV << "[Chord::closestPreceedingNode() @ " << thisNode.getAddress()
                   << " (" << thisNode.getKey().toString(16) << ")]\n"
...
void Chord::recordOverlaySentStats(BaseOverlayMessage* msg)
{
    ...
    while (innerMsg->getType() != APPDATA &&
           innerMsg->getEncapsulatedMsg() != NULL) {
        innerMsg =
            static_cast<BaseOverlayMessage*>(innerMsg->getEncapsulatedMsg());
    }
        ...
        if ((predecessorNode.isUnspecified()) ||
            ...
            if (mergeOptimizationL1) {
                if (!predecessorNode.isUnspecified()) {
                    ...
                    newSuccessorHintMsg->setSrcNode(thisNode);
                    newSuccessorHintMsg->setPreNode(newPredecessor);
                    newSuccessorHintMsg->
                        setBitLength(NEWSUCCESSORHINT_L(newSuccessorHintMsg));
                    sendMessageToUDP(predecessorNode, newSuccessorHintMsg);
                }
...
void Chord::handleRpcNotifyResponse(NotifyResponse* notifyResponse)
{
    ...
    if (successorList->getSuccessor() != notifyResponse->getSrcNode()) {
        EV << "[Chord::handleRpcNotifyResponse() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
...
//{
//    EV << "[Chord::pingResponse() @ " << thisNode.getAddress()
//       << " (" << thisNode.getKey().toString(16) << ")]\n"
...
void Chord::pingTimeout(PingCall* pingCall,
                        ...
                        cPolymorphic* context, int rpcId)
{
    EV << "[Chord::pingTimeout() @ " << thisNode.getAddress()
       << " (" << thisNode.getKey().toString(16) << ")]\n"
...
OverlayKey Chord::distance(const OverlayKey& x, const OverlayKey& y) const
{
