...
void Bamboo::initializeOverlay(int stage)
{
    ...
    routingTableMaintenanceInterval = par("repairTaskTimeoutAmount");
    leafsetMaintenanceInterval = par("leafsetMaintenanceTimeoutAmount");
    globalTuningInterval = par("globalTuningTimeoutAmount");
...
void Bamboo::changeState(int toState)
{
    ...
    case READY:
        ...
        scheduleAt(simTime() + routingTableMaintenanceInterval, localTuningTimer);
...
{
    if (msg == joinTimeout) {
        EV << "[Bamboo::handleTimerEvent() @ " << thisNode.getAddress()
...
void Bamboo::handleUDPMessage(BaseOverlayMessage* msg)
{
    ...
    if (debugOutput) {
        EV << "[Bamboo::handleUDPMessage() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
...
void Bamboo::doGlobalTuning(void)
{
    ...
    while (newKey.getBitRange(maxDigitIndex - digit * bitsPerDigit, bitsPerDigit) ==
        ...
        //std::cout << std::hex << newKey.getBitRange(maxDigitIndex - digit * bitsPerDigit, bitsPerDigit) << " ";
    ...
    //std::cout << lastRow << " " << digit << " " << thisNode.key << " -> " << newKey << std::endl;
}
...
bool Bamboo::handleFailedNode(const TransportAddress& failed)
{
    ...
    if (failed.isUnspecified())
        opp_error("Bamboo::handleFailedNode(): failed is unspecified!");
...
{
    if (debugOutput) {
        EV << "[Bamboo::handleStateMessage() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
...
void Bamboo::lookupFinished(AbstractLookup *lookup)
{
    ...
    if (lookup->isValid()) {
        ...
        if (result[0] != thisNode) {
            // Global Tuning PING
            Prox prox = neighborCache->getProx(result[0], NEIGHBORCACHE_DEFAULT,
                                               PING_SINGLE_NODE, this, NULL);
