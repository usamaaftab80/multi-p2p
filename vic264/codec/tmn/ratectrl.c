...
void InitializeRateControlMB(PictImage *curr,float bit_rate, float target_frame_rate, MotionVector * MV[7][MBR + 1][MBC + 2])
{
  ...
  K_count = 0;
  N = N_frame = (lines>>4) * (pels>>4);
  dev = (float *)calloc((pels>>4)*(lines>>4), sizeof(float));
  alpha = (float *)calloc((pels>>4)*(lines>>4), sizeof(float));
  var = (float *)calloc((pels>>4)*(lines>>4), sizeof(float));
  parameter_count = 0;
  ...
  { 
    for (l=0;l<(pels>>4);l++)
    {
      ...
      for (j=0;j<16;j++)   
        for (i = 0; i < 16; i++) 
          avg += curr->lum[pels*j+l*16 + i+k*pels*16]; 
      for (j = 0;j < 8; j++) 
        for (i = 0; i < 8; i++)
        {
          avg += curr->Cr[(pels>>1)*j+l*8 + i+k*(pels>>1)*8];  
          avg += curr->Cb[(pels>>1)*j+l*8 + i+k*(pels>>1)*8]; 
        }
      avg /= 384;
      ...
      for (j = 0; j < 16; j++) 
        ...
        {
          /* lum */
          s = (float) abs( (int) (curr->lum[pels*j+l*16 + i+k*pels*16] - avg));
          ep += s;
          tmp_var += s*s;
        }
      for (j = 0;j < 8; j++) 
        ...
        {
          /* Cr */
          s = (float) abs( (int) (curr->Cr[(pels>>1)*j+l*8 + i+k*(pels>>1)*8] - avg));
          ep += s;
          tmp_var += s*s;
          /* Cb */
          s = (float) abs( (int) (curr->Cb[(pels>>1)*j+l*8 + i+k*(pels>>1)*8] - avg));
          ep += s;
          tmp_var += s*s; 
        }
         ...
         * algorithm) and divide variance by 3 if Intra coded    */
         var[l+k*(pels>>4)] = (MV[0][k][l]->Mode == MODE_INTRA) ? ((tmp_var-ep*ep/384)/(383))/3 :
                              (tmp_var-ep*ep/384)/(383);
      
        /* Initialize parameter S */
        dev[l+k*(pels>>4)] = (float) sqrt(var[l+k*(pels>>4)]);
        alpha[l+k*(pels>>4)] = ((B_target/256)/N_frame < 0.5) ? 
          B_target/(128*N_frame) * (1-dev[l+k*(pels>>4)]) + dev[l+k*(pels>>4)] : 1; 
        S += alpha[l+k*(pels>>4)] * dev[l+k*(pels>>4)];
    }
  }
...
#if 0
  fprintf(stdout,"\nQP(%3d): %3d K: %5.4f B_target: %6.0f dev: %5.2f alpha: %4.3f  L: %5.0f C: %5.4f, S: %5.0f",xpos+ypos * (pels>>4), newQP, K, B_target, dev[xpos+ypos * (pels>>4)], alpha[xpos+ypos * (pels>>4)], L, C, S );
#endif
