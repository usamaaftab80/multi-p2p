...
void BasePastry::purgeVectors(void)
{
    ...
    // purge Queue for messages to be forwarded in recursive mode:
    for (std::map<TransportAddress, BaseRouteMessage*>::iterator i =
        recFwdQueue.begin(); i != recFwdQueue.end(); i++) {
        delete i->second;
    }
    recFwdQueue.clear();
...
void BasePastry::baseInit()
{
    ...
    optimisticForward = par("optimisticForward");
    avoidDuplicates = par("avoidDuplicates");
    sendStateWaitAmount = par("sendStateWaitAmount");
    timeoutPing = par("pingTimeout");
    pingRetries = par("pingRetries");
    useRegularNextHop = par("useRegularNextHop");
    alwaysSendUpdate = par("alwaysSendUpdate");

    useDiscovery = false; //TODO
    ...
    if (numberOfLeaves % 2) {
        EV << "[BasePastry::baseInit() @ " << thisNode.getAddress()
...
void BasePastry::baseChangeState(int toState)
{
    ...
    case INIT:
        ...
        routingTable->initializeTable(bitsPerDigit, repairTimeout,
                                      thisNode);
        leafSet->initializeSet(numberOfLeaves, bitsPerDigit,
                               repairTimeout, thisNode,
                               this);
...
void BasePastry::newLeafs(void)
{
    ...
    if (msg) {
        send(msg, "appOut");
        EV << "[BasePastry::newLeafs() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    newLeafs() called."
           << endl;
    }
...
bool BasePastry::recursiveRoutingHook(const TransportAddress& dest,
                                      BaseRouteMessage* msg)
{
    ...
    PastryMessage* pmsg =
        dynamic_cast<PastryMessage*>(msg->getEncapsulatedMsg());
    ...
    if (pmsg && pmsg->getPastryMsgType() == PASTRY_MSG_JOIN) {
        ...
        if (jmsg->getSendStateTo() != thisNode) {
            ...
            sendStateTables(jmsg->getSendStateTo(), PASTRY_STATE_JOIN,
                            check_and_cast<OverlayCtrlInfo*>(msg->getControlInfo())
                            ->getHopCount(), false);
        }
    }

    if (recFwdQueue.find(dest) != recFwdQueue.end()) {
        // rare case, other message for same next hop is pending:
        // send the message and hope it isn't lost.
        return true;
    }

    assert(msg->getControlInfo());

    if (optimisticForward) {
        // forward now:
        return true;
    } else {
        if (routeMsgAcks) throw cRuntimeError("!optimisticForward && "
                                                  "routeMsgAcks");
        // keep message in queue (forward later)
        recFwdQueue[dest] = msg;
        pingNode(dest, timeoutPing, pingRetries, NULL,
                 "PING next hop", NULL, PING_NEXT_HOP);

        return false;
    }
}

int BasePastry::getNextRowToMaintain()
{
    int digit = 0;
...
void BasePastry::doRoutingTableMaintenance()
{
    if (sendPullFlag) {
        // bamboo
        rowToAsk = getNextRowToMaintain();
        //uint32_t rrows = routingTable->getLastRow();
        //if (++rowToAsk >= rrows) rowToAsk = 0;
        const TransportAddress& ask4row = routingTable->getRandomNode(rowToAsk);

        if ((!ask4row.isUnspecified()) && (ask4row != thisNode)) {
            PastryRoutingRowRequestMessage* msg = new PastryRoutingRowRequestMessage("ROWREQ");
            ...
            RECORD_STATS(routingTableReqSent++; routingTableReqBytesSent += msg->getByteLength());

            EV << "[BasePastry::doRoutingTableMaintenance() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
...
{
    EV << "[BasePastry::pingResponse() @ " << thisNode.getAddress()
       << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    if (rpcId == PING_SINGLE_NODE) {
        routingTable->mergeNode(src, rtt);
        return;
    }

    /*// a node with the an equal ID has responded
    if ((src.getKey() == thisNode.getKey()) && (src.getAddress() != thisNode.getAddress())) {
        EV << "[BasePastry::pingResponse() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
        ...
        switch (pingContext->stateObject) {
            case ROUTINGTABLE: {
                ...
                if((node->isUnspecified()) || (*node != src)) {
                    std::cout << simTime() << " " << thisNode.getAddress() << " rt: state from "
                              << stateCache.msg->getSender().getAddress() << " *** failed: node "
                              << node->ip << " src " << src.getAddress() << std::endl;
                    break;
                }*/
            ...
            case LEAFSET: {
                ...
                if ((node->isUnspecified()) || (*node != src)) {
                    std::cout << simTime() << " " << thisNode.getAddress() << " ls: state from "
                              << stateCache.msg->getSender().getAddress() << " *** failed: node "
                              << node->ip << " src " << src.getAddress() << std::endl;
                    break;
                }*/
            ...
            case NEIGHBORHOODSET: {
                ...
                if((node->isUnspecified()) || (*node != src)) {
                    std::cout << simTime() << " " << thisNode.getAddress() << " ns: state from "
                              << stateCache.msg->getSender().getAddress() << " *** failed: node "
                              << node->ip << " src " << src.getAddress() << std::endl;
                    break;
                }*/
    ...
    std::map<TransportAddress, BaseRouteMessage*>::iterator pos;
    if ((pos = recFwdQueue.find(src)) != recFwdQueue.end()) {
        // send message
        if (!optimisticForward) {
            sendMessageToUDP(pos->first, pos->second);
        } else if (!avoidDuplicates) {
            delete pos->second;
        }
                              ...
                              cPolymorphic *contextPointer, Prox prox)
{
    ...
    EV << "[BasePastry::proxCallback() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    Pong received (from "
           << node.getAddress() << ")"
           << endl;
    ...
    // merge single pinged nodes (bamboo global tuning)
    if (rpcId == PING_SINGLE_NODE) {
        routingTable->mergeNode((const NodeHandle&)node, rtt);
        return;
    }
...
{
    EV << "[BasePastry::pingNodes() @ " << thisNode.getAddress()
...
void BasePastry::sendStateTables(const TransportAddress& destination, int type, ...)
{
    if (destination.getAddress() == thisNode.getAddress())
    ...
    if ((type == PASTRY_STATE_JOIN) || (type == PASTRY_STATE_UPDATE)) {
    ...
    if (type == PASTRY_STATE_JOIN) {
        ...
        EV << "[BasePastry::sendStateTables() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    if ((type == PASTRY_STATE_JOIN) && (useDiscovery)) { //TODO remove dependency on discovery mode
        routingTable->dumpRowToMessage(stateMsg, hops); //send just the needed row for new join protocol
...
void BasePastry::sendStateDelayed(const TransportAddress& destination)
{
    ...
    scheduleAt(simTime() + sendStateWaitAmount, selfMsg);
}
...
void BasePastry::pingTimeout(PingCall* msg,
                             ...
                             int rpcId)
{
    EV << "[BasePastry::sendStateDelayed() @ " << thisNode.getAddress()
       << " (" << thisNode.getKey().toString(16) << ")]\n"
       << "    Ping timeout occurred (" << dest.getAddress() << ")"
       << endl;
        ...
        switch (pingContext->stateObject) {
            case ROUTINGTABLE: {
                ...
                if((node->isUnspecified()) || (dest != *node)) {
                    std::cout << msg->getNonce() << " " << simTime() << " " << thisNode.getAddress() << " rt: state from "
                    << stateCache.msg->getSender().getAddress() << " *** failed: node "
                    << node->ip << " failed dest " << dest.getAddress() << std::endl;
                    break;
                }*/
            ...
            case LEAFSET: {
                ...
                node = &(stateCache.msg->getLeafSet(pingContext->index));
                if((node->isUnspecified()) || (dest != *node)) {
                    std::cout << msg->getNonce() << " " << simTime() << " " << thisNode.getAddress() << " ls: state from "
                    << stateCache.msg->getSender().getAddress() << " *** failed: node "
                    << node->ip << " failed dest " << dest.getAddress() << std::endl;
                    break;
                }*/
            ...
            case NEIGHBORHOODSET: {
                ...
                node = &(stateCache.msg->getNeighborhoodSet(pingContext->index));
                if((node->isUnspecified()) || (dest != *node)) {
                    std::cout << msg->getNonce() << " " << simTime() << " " << thisNode.getAddress() << " ns: state from "
                    << stateCache.msg->getSender().getAddress() << " *** failed: node "
                    << node->ip << " failed dest " << dest.getAddress() << std::endl;
                    break;
                }*/
    ...
    if (rpcId == PING_NEXT_HOP) {
        // handle forward queue entry
        std::map<TransportAddress, BaseRouteMessage*>::iterator pos;
        if ((pos = recFwdQueue.find(dest)) == recFwdQueue.end()) return;
        BaseRouteMessage* rmsg = pos->second;
        recFwdQueue.erase(pos);
        if (dynamic_cast<PastryJoinMessage*>(rmsg->getEncapsulatedMsg()) &&
                rmsg->getDestKey() == thisNode.getKey()) {
            //std::cout << "join timeout -> rejoin!";
            changeState(INIT);
            return;
        }
        assert(rmsg->getControlInfo());
        sendToKey(rmsg->getDestKey(), rmsg);
    }
}
...
void BasePastry::sendRequest(const TransportAddress& ask, int type)
{
    ...
    case PASTRY_REQ_REPAIR:
        if (ask.isUnspecified())
            opp_error("Pastry::sendRequest(): asked for repair from "
                  "unspecified node!");
        msgName += "Repair";
        break;
    ...
    case PASTRY_REQ_STATE:
        if (ask.isUnspecified())
            opp_error("Pastry::sendRequest(): asked for state from "
                  "unspecified node!");
        msgName += "State";
        break;
    ...
    case PASTRY_REQ_LEAFSET:
        if (ask.isUnspecified())
            opp_error("Pastry::sendRequest(): asked for leafset from "
                  "unspecified node!");
        msgName += "Leafset";
        break;
    }
...
void BasePastry::handleRequestMessage(PastryRequestMessage* msg)
{
    ...
    if (reqtype == PASTRY_REQ_REPAIR) {
        ...
        if (state == READY)
            ...
            EV << "[BasePastry::handleRequestMessage() @ " << thisNode.getAddress()
            << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    if (state != READY) {
        ...
        if (findNodeExt) {
            ...
            if (!stateRecipient.isUnspecified()) {
                ...
                sendStateTables(stateRecipient, PASTRY_STATE_JOIN,
                                findNodeExt->getJoinHopCount(), false);
            }
        }

        next = &(leafSet->getDestinationNode(key));
                                         ...
                                         bool appLookup)
{
    ...
    if (msg) {
        const PastryMessage* pmsg =
            dynamic_cast<const PastryMessage*>(msg->getEncapsulatedMsg());
        if ((pmsg) && (pmsg->getPastryMsgType() == PASTRY_MSG_JOIN)) {
