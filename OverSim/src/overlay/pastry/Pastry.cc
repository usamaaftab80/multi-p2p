...
void Pastry::initializeOverlay(int stage)
{
    ...
    routingTableMaintenanceInterval = par("repairTaskTimeoutAmount"); // TODO parameter into derived classes
    sendStateAtLeafsetRepair = par("sendStateAtLeafsetRepair");
    ringCheckInterval = par("ringCheckInterval");
    ...
    ringCheck = new cMessage("ringCheck");

    if (useDiscovery) discoveryTimeout = new cMessage("discoveryTimeout");
    if (routingTableMaintenanceInterval > 0) repairTaskTimeout =
        new cMessage("repairTaskTimeout");
...
{
    if (ringCheck->isScheduled()) cancelEvent(ringCheck);
    ...
    case DISCOVERY:
        ...
        sendRequest(bootstrapNode, PASTRY_REQ_LEAFSET);
        depth = -1;
    ...
    case READY:
        if (ringCheckInterval > 0) {
            scheduleAt(simTime()+ringCheckInterval, ringCheck);
        }
        ...
        scheduleAt(simTime() + sendStateWaitAmount, joinUpdateWait);
...
void Pastry::handleTimerEvent(cMessage* msg)
{
    ...
    if (msg->isName("joinTimeout")) {
        EV << "[Pastry::handleTimerEvent() @ " << thisNode.getAddress()
        << " (" << thisNode.getKey().toString(16) << ")]\n"
        << "    timeout expired, restarting..."
        << endl;
        //std::cout << simTime() << " " << thisNode.getAddress() << " " << stReceived.size() <<" Pastry: join timeout expired, restarting..." << std::endl;
        join();
    } else if (msg->isName("readyWait")) {
        if (partialJoinPath) {
            ...
            EV << "[Pastry::handleTimerEvent() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    timeout waiting for missing state messages in JOIN state, restarting..."
               << endl;
            //std::cout << thisNode.getAddress() << "Pastry: timeout waiting for missing state messages in JOIN "
            //                "state, restarting..." << std::endl;
            join();
        }
    } else if (msg->isName("joinUpdateWait")) {
        EV << "[Pastry::handleTimerEvent() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    sending state updates to all nodes."
           << endl;
        doJoinUpdate();
    } else if (msg->isName("secondStageWait")) {
        EV << "[Pastry::handleTimerEvent() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    sending STATE requests to all nodes in second stage of initialization."
           << endl;
        doSecondStage();
    } else if (msg->isName("ringCheck")) {
        if (state == READY) {
            // ping direct neighbors on the ring:
            const NodeHandle& pred = leafSet->getPredecessor();
            const NodeHandle& succ = leafSet->getSuccessor();
            if (! pred.isUnspecified()) {
                pingNode(pred, timeoutPing, pingRetries,
                         NULL, "PING ring check");
            }
            if (! succ.isUnspecified()) {
                pingNode(succ, timeoutPing, pingRetries,
                         NULL, "PING ring check");
            }
        }
        scheduleAt(simTime() + ringCheckInterval, ringCheck);
        ...
        delete sendStateMsg;
    } else if (msg->isName("discoveryTimeout")) {
    ...
    } else if (msg->isName("repairTaskTimeout")) {
        EV << "[Pastry::handleTimerEvent() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    case PASTRY_MSG_JOIN: {
        ...
        if (state != READY) {
            ...
            sendStateTables(jmsg->getSendStateTo(),
                            PASTRY_STATE_JOIN, joinHopCount, true);
        }
...
void Pastry::doSecondStage(void)
{
    ...
    for (std::vector<TransportAddress>::iterator it = notifyList.begin();
        ...
        EV << "[Pastry::doSecondStage() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
...
bool Pastry::handleFailedNode(const TransportAddress& failed)
{
    ...
    //std::cout << thisNode.getAddress() << " is handling failed node: " << failed.getAddress() << std::endl;
    ...
    if (wasValid && lsAsk.isUnspecified() && (! leafSet->isValid())) {
        EV << "[Pastry::handleFailedNode() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
           << "    lost connection to the network, trying to re-join."
           << endl;
        //std::cout << thisNode.getAddress() << " Pastry: lost connection to the network, trying to re-join."
        //          << std::endl;
        join();
        return false;
    }
...
{
    //std::cout << "checkProxCache()" << stateCache.msg << " " << stateCache.prox << std::endl;
    EV << "[Pastry::checkProxCache() @ " << thisNode.getAddress()
       << " (" << thisNode.getKey().toString(16) << ")]"
       << endl;
    ...
    EV << "[Pastry::checkProxCache() @ " << thisNode.getAddress()
       ...
       << stateCache.msg->getSender().getAddress()
       << " collected!"
       << endl;
    /*
    //debug
...
*/
    ...
    if (state == JOINING_2) {
        // save pointer to proximity vectors:
        ...
        if (++stReceivedPos == stReceived.end()) {
            ...
            for (uint32_t i = 0; i < stReceived.size(); ++i) {
                EV << ((i == 0) ? "    " : " | ");
                if (stReceived[i].msg == stReceivedPos->msg) EV << "*";
                EV << stReceived[i].msg;
            }
            EV << endl;
...
void Pastry::endProcessingState(void)
{
    ...
    /*else {
        std::cout << thisNode.getAddress() << "\t" << simTime()
...
bool Pastry::mergeState(void)
{
    ...
    if (state == JOINING_2) {
        ...
        if (debugOutput) {
            EV << "[Pastry::mergeState() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    [JOIN] starting to build own state from "
               << stReceived.size() << " received state messages..."
               << endl;
        ...
        if (stateCache.msg &&
            ...
            if (!neighborhoodSet->mergeState(stReceived.front().msg,
                ...
                EV << "[Pastry::mergeState() @ " << thisNode.getAddress()
                   << " (" << thisNode.getKey().toString(16) << ")]\n"
                   << "    Error initializing own neighborhoodSet while joining! Restarting ..."
                   << endl;
                ret = false;
            }
        }
        ...
        if (!leafSet->mergeState(stReceived.back().msg,
            ...
            EV << "[Pastry::mergeState() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    Error initializing own leafSet while joining! Restarting ..."
               << endl;
        ...
        if (!routingTable->initStateFromHandleVector(stReceived)) {
            EV << "[Pastry::mergeState() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    Error initializing own routingTable while joining! Restarting ..."
               << endl;
        ...
        // merging singe state (stateCache.msg)
...
{
    if (debugOutput) {
        EV << "[Pastry::handleStateMessage() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
    ...
    if (state == JOINING_2) {
        ...
        if (pingBeforeSecondStage) prePing(msg);
        ...
        if (msg->getLastHop()) {
            ...
            if (joinHopCount) {
                EV << "[Pastry::handleStateMessage() @ " << thisNode.getAddress()
                   ...
                   << endl;
                //std::cout << thisNode.getAddress() << "Pastry: Error: received a second `last' state message! "
                //                    "Restarting ..." << std::endl;
                join();
                return;
            }
            ...
            if (stReceived.size() < joinHopCount) {
                ...
                //std::cout << simTime() << " " << thisNode.getAddress() << " readyWait scheduled!" << std::endl;
            }
        }
        ...
        if (joinHopCount) {
            ...
            if (stReceived.size() == joinHopCount) {
                ...
                EV << "[Pastry::handleStateMessage() @ " << thisNode.getAddress()
                   ...
                   << endl;
                if (pingBeforeSecondStage) pingNodes();
                else {
                    mergeState(); // JOINING / stateCache
                    //endProcessingState(); no way
                    //stateCache.msg = NULL; //test
                    changeState(READY);
                    EV << "[Pastry::handleStateMessage() @ " << thisNode.getAddress()
                       << " (" << thisNode.getKey().toString(16) << ")]\n"
                       << "    changeState(READY) called"
