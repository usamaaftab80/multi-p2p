...
std::ostream& operator<<(std::ostream& Stream, const Site s)
{
    ...
    return Stream << "  IP: " << s.addr.getAddress();
}
...
void Geometry::processEdge(Edge *e)
{
    ...
    if(e->reg[le]->type == THIS) {
        ...
        if(debugOutput) EV << "[Geometry::processEdge()]\n"
                           << "    Site at [" << e->reg[re]->coord.x << ", " << e->reg[re]->coord.y << "] is a enclosing neighbor."
                           << endl;
    }
    if(e->reg[re]->type == THIS) {
        ...
        // Debug output
        if(debugOutput) EV << "[Geometry::processEdge()]\n"
                           << "    Site at [" << e->reg[le]->coord.x << ", " << e->reg[le]->coord.y << "] is a enclosing neighbor."
                           << endl;
    }
}
