...
{
    EV << "[Broose::displayBucketState() @ " << thisNode.getAddress()
...
void Broose::updateTooltip()
{
    if (ev.isGUI()) {
        ...
        ttString << thisNode.getAddress() << " " << thisNode.getKey();
                               ...
                               simtime_t rtt)
{
    ...
    RPC_ON_RESPONSE( Bucket ) {
        ...
        EV << "[Broose::handleRpcResponse() @ " << thisNode.getAddress()
...
void Broose::handleRpcTimeout(const RpcState& rpcState)
{
    ...
    RPC_ON_CALL(FindNode) {
        ...
        EV << "[Broose::handleRpcTimeout() @ " << thisNode.getAddress()
        << " (" << thisNode.getKey().toString(16) << ")]\n"
