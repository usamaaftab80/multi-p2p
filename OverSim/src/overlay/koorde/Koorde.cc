...
bool Koorde::handleRpcCall(BaseCallMessage* msg)
{
    if (state == READY) {
        ...
        EV << "[Koorde::handleRpcCall() @ " << thisNode.getAddress()
                               ...
                               int rpcId, simtime_t rtt)
{
    ...
    RPC_ON_RESPONSE( DeBruijn ) {
        ...
        EV << "[Koorde::handleRpcResponse() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
                              ...
                              const OverlayKey& destKey)
{
    ...
    RPC_ON_CALL( DeBruijn ) {
        ...
        EV << "[Koorde::handleRpcTimeout() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
...
void Koorde::updateTooltip()
{
    ...
    if (ev.isGUI()) {
        ...
        for (unsigned int i = 0; i < successorList->getSize(); i++) {
            ttString << successorList->getSuccessor(i).getAddress() << " ";
        }
        ...
        for (int i = 0; i < deBruijnNumber; i++) {
            ttString << deBruijnNodes[i].getAddress() << " ";
        }
