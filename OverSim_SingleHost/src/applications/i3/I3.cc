...
void I3::insertTrigger(I3Trigger &t)
{
    ...
    if (t.getIdentifierStack().size() == 0) {
        /* don't bother */
        cout << "Warning: Got trigger " << t << " with size 0 in " << thisNode.getAddress()<< endl;
        return;
    }
    ...
    if (id.getType() == I3SubIdentifier::IPAddress) {
        ...
        if (!i3id) {
            ...
            if (idStack.size() == 0) {
                ...
                numDroppedPackets++;
                byteDroppedPackets += msg->getBitLength();
                cout << "Dropped packet at" << thisNode.getAddress()<< " to unknown id " << id.getIdentifier() << endl;
                delete msg;
                return;
            } else {
            ...
            for (it = s.begin(); it != s.end(); it++) {
                ...
                dupMsg = check_and_cast<cPacket*>(msg->getEncapsulatedMsg()->dup()); /* dup msg */
...
void I3::handleUDPMessage(cMessage *msg)
{
    ...
    case INSERT_TRIGGER:
        ...
        /*            if ((imsg->getSource().address.d[0] & 0xff) == 56) {
                    cout << "UDP Server " << thisNode.getAddress()<< " trigger " << imsg->getTrigger() << " key " << key << endl;
            }*/
...
void I3::sendQueryReply(const I3Identifier &id, const I3IPAddress &add) {
    ...
    I3IPAddress myAddress(thisNode.getAddress(), par("serverPort"));
        ...
        for (I3TriggerTable::iterator it = triggerTable.begin(); it != triggerTable.end(); it++) {
            ...
            for (set<I3Trigger>::const_iterator sit = triggerSet.begin(); sit != triggerSet.end(); sit++) {
                ...
                if (simTime() - sit->getInsertionTime() > triggerTimeToLive) {
                    ...
                    //		thisNode.getAddress()<< ", insertion time is " << sit->getInsertionTime()<< endl;
                    //}
