...
void KBRTestApp::initializeApp(int stage)
{
    ...
    if (!kbrOneWayTest && !kbrRpcTest && !kbrLookupTest) {
        throw cRuntimeError("KBRTestApp::initializeApp(): nothing to do!");
    }
                                   ...
                                   simtime_t rtt)
{
    ...
    RPC_ON_RESPONSE(Lookup) {
        EV << "[KBRTestApp::handleRpcResponse() @ " << overlay->getThisNode().getAddress()
...
{
    EV << "[KBRTestApp::handleLookupResponse() @ " << overlay->getThisNode().getAddress()
       << " (" << overlay->getThisNode().getKey().toString(16) << ")]\n"
...
void KBRTestApp::deliver(OverlayKey& key, cMessage* msg)
{
    ...
    if ((msgHandleBufSize > 0 )
        ...
        EV << "[KBRTestApp::deliver() @ " << overlay->getThisNode().getAddress()
           << " (" << overlay->getThisNode().getKey().toString(16) << ")]\n"
           << "    Duplicate dropped."
           << endl;
...
OverlayKey KBRTestApp::createDestKey()
{
    if (lookupNodeIds) {
        return globalNodeList->getRandomNode(0, true, true).getKey();
    }
...
void KBRTestApp::evaluateData(simtime_t latency, int hopCount, long int bytes)
{
    ...
    if (numSent < numDelivered) {
        error("KBRTestApp::evaluateData(): numSent < numDelivered!");
    }
