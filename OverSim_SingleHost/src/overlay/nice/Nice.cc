...
Nice::Nice()
{
	...
	char str[80];
	fscanf(rpfile,"%s",str);
	RendevouzPoint = TransportAddress(IPvXAddress(str),1024,TransportAddress::UNKNOWN_NAT);
	fclose(rpfile);
...
//                    globalStatistics->recordOutVector("1 Distance",distance);
        ...
        //hoang
			...
			NiceMessage * msg = new NiceMessage("NICE_RP_PING");

			msg->setCommand(NICE_RP_PING);
			msg->setSrcNode(thisNode);
