...
def plotHull(filter, text):
    ...
    if options.ci == 0:
        return ax.plot(x, y, label=text, linestyle=linecycler(), color=colorcycler(),
                       marker=markercycler(), lw=1.5, markersize=9.0, markeredgewidth=2.0)[0]
    else:
...
allHull.set_visible(options.hull)
...
def toggleHullVisibility(label):
    ...
    elif label=='all': allHull.set_visible(not allHull.get_visible())
    elif label=='config':
...
#    fig.savefig(options.outfile)
    ...
    for h in configHulls:
        if h.get_visible():
            outfile.write("\"" + options.outfile+".dat\" index "+str(plotidx)+" title \""+h.get_label()+"\",")
            datfile.write("# " + h.get_label() + "\n")
            np.savetxt(datfile, zip(h.get_xdata().data, h.get_ydata().data), delimiter=',')
            datfile.write("\n\n")
            plotidx += 1
    for i in iterHulls:
        for j in iterHulls[i]:
            if j.get_visible():
                outfile.write("\"" + options.outfile+".dat\" index "+str(plotidx)+" title \""+j.get_label()+"\",")
                datfile.write("# " + j.get_label() + "\n")
                np.savetxt(datfile, zip(j.get_xdata().data, j.get_ydata().data), delimiter=',')
                datfile.write("\n\n")
                plotidx += 1
